...
import { UserProfile, Project, Protocol } from "@/lib/types";
import {
        ...
        <div className="p-8 pb-6 overflow-y-auto">
          ...
          {step === 3 && (
            <Step3SiteDetails
              ...
              isMicroZone={isMicroZone}
              setIsMicroZone={setIsMicroZone}
              microZoneName={microZoneName}
              setMicroZoneName={setMicroZoneName}
          ...
          {step === 4 && (
            <Step4Review
              ...
              setAssignToManager={setAssignToManager}
...
interface Step2ProtocolProps {
  ...
  setIsNewProtocol: (val: boolean) => void;
  selectedProtocolId: string;
  setSelectedProtocolId: (val: string) => void;
  newProtocolName: string;
  setNewProtocolName: (val: string) => void;
  protocols: Protocol[];
  selectedProjectId: string;
  handleNextStep: () => void;
}

function Step2Protocol({
  isNewProtocol,
  setIsNewProtocol,
  selectedProtocolId,
  setSelectedProtocolId,
      ...
      <div className="flex flex-col gap-4">
        <label className={`p-4 border-[1.5px] rounded-xl cursor-pointer transition-all ${!isNewProtocol ? 'border-indigo-600 bg-indigo-50/50' : 'border-neutral-200 hover:border-indigo-300'}`}>
          ...
          {!isNewProtocol && (
            ...
            >
              <option value="">-- Seleccionar --</option>
              {protocols.filter((p: Protocol) => p.project_id === selectedProjectId).map((p: Protocol) => (
...
interface Step3SiteDetailsProps {
  siteNumber: string;
  setSiteNumber: (val: string) => void;
  siteName: string;
  setSiteName: (val: string) => void;
  siteAddress: string;
  setSiteAddress: (val: string) => void;
  siteCity: string;
  setSiteCity: (val: string) => void;
  siteCountry: string;
  handleCountryChange: (val: string) => void;
  isMicroZone: boolean;
  setIsMicroZone: (val: boolean) => void;
  microZoneName: string;
  setMicroZoneName: (val: string) => void;
  ALL_COUNTRIES: string[];
  COUNTRY_TO_REGION: Record<string, string>;
}

function Step3SiteDetails({
  siteNumber, setSiteNumber,
  siteName, setSiteName,
  siteAddress, setSiteAddress,
  siteCity, setSiteCity,
  siteCountry, handleCountryChange,
  isMicroZone, setIsMicroZone,
  microZoneName, setMicroZoneName,
  ALL_COUNTRIES, COUNTRY_TO_REGION
}: Step3SiteDetailsProps) {
  return (
    <div className="animate-in fade-in slide-in-from-right-4 duration-300">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <select value={siteCountry} onChange={e => handleCountryChange(e.target.value)} className="w-full px-3 py-2 border border-neutral-300 rounded-lg text-[13.5px] focus:outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10">
              <option value="">-- Seleccionar --</option>
              {ALL_COUNTRIES.map((c: string) => <option key={c} value={c}>{c}</option>)}
            </select>
          </div>
        </div>
...
interface Step4ReviewProps {
  ...
  setAssignToManager: (val: boolean) => void;
...
function Step4Review({
  isNewProject, newProjectName, projects, selectedProjectId,
  isNewProtocol, newProtocolName, protocols, selectedProtocolId,
  siteNumber, siteName, siteAddress, siteCity, siteCountry, isMicroZone, microZoneName, COUNTRY_TO_REGION,
  assignToManager, setAssignToManager, selectedManagerId, setSelectedManagerId, profiles
}: Step4ReviewProps) {
  return (
    <div className="animate-in fade-in slide-in-from-right-4 duration-300">
        ...
        <div className="flex gap-3">
          <div className="flex-1 p-3 bg-neutral-50 border border-neutral-200 rounded-xl">
            ...
            <div className="text-[14px] font-semibold text-neutral-900 mt-0.5 flex items-center gap-1.5">
              {isNewProject ? newProjectName : projects.find((p: Project) => p.id === selectedProjectId)?.name}
          ...
          <div className="flex-1 p-3 bg-neutral-50 border border-neutral-200 rounded-xl">
            ...
            <div className="text-[14px] font-semibold text-neutral-900 mt-0.5 flex items-center gap-1.5">
              {isNewProtocol ? newProtocolName : protocols.find((p: Protocol) => p.id === selectedProtocolId)?.name}
          ...
          {assignToManager && (
            <div className="mt-3 pl-6 animate-in fade-in slide-in-from-top-1">
              <select value={selectedManagerId} onChange={e => setSelectedManagerId(e.target.value)} className="w-full px-3 py-2 border border-neutral-300 bg-white rounded-lg text-[13px] focus:outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10">
                ...
                {profiles.filter((p: UserProfile) => p.role === 'manager').map((m: UserProfile) => (
  ...
  return (
    <div className="px-8 py-4 bg-neutral-50/80 border-t border-neutral-200 flex items-center justify-between shrink-0">
      ...
      <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
        {step > 1 && (
          <button onClick={() => setStep((s: number) => s - 1)} disabled={loading} className="px-3.5 py-1.5 text-[13px] font-medium text-neutral-500 hover:text-neutral-800 transition-colors disabled:opacity-50">
            ← Atrás
          </button>
        )}
